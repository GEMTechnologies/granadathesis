...
from core.events import events  # Import at module level
...
class CitedContentGenerator:
    ...
    def __init__(
        ...
        # Initialize agent pool - use configurable max_concurrent
        from core.config import settings
        self.agent_pool = AgentPool(max_concurrent=settings.MAX_CONCURRENT_AGENTS)
    ...
    async def generate_cited_section(
        ...
        job_id: Optional[str] = None  # For event emission
    ) -> Dict[str, Any]:
        """
        ...
        if job_id:
            await events.log(job_id, f"📚 Searching academic literature for: {topic}...")
            await events.debate_message(job_id, "CitationFinder", f"Initiating search for relevant papers on '{topic}'...")
        
        print("🔍 Step 1: Searching for relevant papers...")
        papers = await self._search_papers(topic, max_results=20, job_id=job_id)
        print(f"   ✓ Found {len(papers)} papers\n")
        
        if job_id and papers:
            await events.log(job_id, f"✓ Discovered {len(papers)} relevant papers", "success")
            # Emit graph nodes for papers
            for i, paper in enumerate(papers[:5]):  # Show first 5 in graph
                await events.publish(job_id, "graph_node", {
                    "id": f"paper_{i}",
        ...
        if job_id:
            await events.log(job_id, f"✍️ Generating {int(word_count / 15)} cited sentences...")
            await events.debate_message(job_id, "SentenceWriter", f"Beginning sentence generation with citation integration...")
        ...
        for i in range(target_sentences):
            ...
            if should_cite and papers:
                ...
                # Emit sentence to UI for real-time streaming
                if job_id:
                    await events.publish(job_id, "content_chunk", {
                        "sentence": sentence_data['sentence'],
                        "index": i,
                        "total": target_sentences
                    })
            else:
            ...
            if (i + 1) % 5 == 0:
                ...
                if job_id:
                    await events.publish(job_id, "progress", {
                        ...
                        "percent": int((i + 1) / target_sentences * 100)
                    })
                    if (i + 1) % 10 == 0:
                        await events.debate_message(job_id, "SentenceWriter", 
                            f"Progress: {i + 1}/{target_sentences} sentences complete ({current_word_count} words)")
        ...
        if job_id:
            await events.log(job_id, "📊 Analyzing citation density...")
            await events.debate_message(job_id, "DensityAnalyzer", "Calculating citation metrics and validation...")
        ...
        if job_id:
            await events.log(job_id, f"📚 Generating reference list for {len(self.cited_papers)} papers...")
            await events.debate_message(job_id, "ReferenceGenerator", f"Formatting {len(self.cited_papers)} references in {self.citation_style} style...")
        ...
        if job_id:
            await events.log(job_id, f"✅ Section complete! {current_word_count} words, {len(self.cited_papers)} citations ({density_data.get('citation_density', 0):.1%} density)", "success")
            await events.stage_completed(job_id, "content_generation", {
    ...
    async def _search_papers(
        ...
        papers = await academic_search_service.search_academic_papers(
            ...
            job_id=job_id  # Pass job_id for event emission
        )
        return papers
