...
class RedFlagDetector:
    ...
    def __init__(
        ...
        # Methodology creep patterns (from objective.py) - STRICT
        self.methodology_patterns = [
            ...
            r'\b(quantitative\s+analysis)\b',
            r'\b(qualitative\s+analysis)\b',
            r'\b(data\s+collection)\b',
            r'\b(sampling\s+method)\b',
        ]
        
        # Vague language patterns - ENHANCED
        self.vague_patterns = [
        ...
        # Undergraduate-level weak language patterns - NEW
        self.weak_language_patterns = [
            r'\b(to\s+find\s+out)\b',
            r'\b(to\s+see\s+if)\b',
            r'\b(to\s+check)\b',
            r'\b(to\s+look\s+at)\b',
            r'\b(to\s+see)\b',
            r'\b(to\s+count)\b',
            r'\b(to\s+list)\b',
            r'\b(how\s+many)\b',
        ...
        # Trivial/too narrow patterns - NEW
        self.trivial_patterns = [
            r'\b(to\s+count\s+how\s+many)\b',
            r'\b(to\s+list\s+all)\b',
            r'\b(to\s+describe\s+what)\b',
            r'\b(to\s+name\s+the)\b',
        ]
        
        # Too broad/impossible patterns - NEW
        self.too_broad_patterns = [
            r'\b(to\s+solve\s+all)\b',
            r'\b(to\s+understand\s+everything)\b',
            r'\b(to\s+explore\s+all\s+aspects)\b',
            r'\b(comprehensive\s+analysis\s+of\s+everything)\b',
        ]
    ...
    def detect_flags(self, response: str, context: Optional[Dict[str, Any]] = None) -> RedFlagResult:
        ...
        # 3. Academic quality checks (for objectives) - ENHANCED
        if self.enable_academic_check and task_type == 'objective':
            ...
            if academic_issues:
                ...
                # Weak language or trivial objectives are critical for PhD
                if any('weak language' in issue.lower() or 'trivial' in issue.lower() or 'too broad' in issue.lower() for issue in academic_issues):
                    severity = 'critical'
    ...
    def _check_academic_quality(self, response: str) -> List[str]:
        ...
        Focuses on methodology creep, vague language, weak language, and PhD-level rigor.
        """
        issues = []
        
        # Check for methodology creep (CRITICAL - automatic rejection)
        methodology_matches = []
        ...
        if methodology_matches:
            issues.append(f"CRITICAL: Methodology creep detected: {', '.join(set(methodology_matches[:3]))}")
        
        # Check for weak/undergraduate-level language (CRITICAL for PhD)
        weak_matches = []
        ...
        if weak_matches:
            issues.append(f"CRITICAL: Weak/undergraduate-level language detected: {', '.join(set(weak_matches))}")
        
        # Check for trivial/too narrow objectives (CRITICAL for PhD)
        trivial_matches = []
        for pattern in self.trivial_patterns:
            matches = re.findall(pattern, response, re.IGNORECASE)
            if matches:
                trivial_matches.extend(matches)
        
        if trivial_matches:
            issues.append(f"CRITICAL: Trivial/too narrow objectives detected: {', '.join(set(trivial_matches))}")
        
        # Check for too broad/impossible objectives (CRITICAL for PhD)
        broad_matches = []
        for pattern in self.too_broad_patterns:
            matches = re.findall(pattern, response, re.IGNORECASE)
            if matches:
                broad_matches.extend(matches)
        
        if broad_matches:
            issues.append(f"CRITICAL: Too broad/impossible objectives detected: {', '.join(set(broad_matches))}")
        ...
        if self._is_too_vague(response):
            issues.append("Objectives too vague (SMART violation - not PhD standard)")
        
        # Check for appropriate length (PhD objectives are 20-35 words)
        objectives = self._extract_objectives(response)
        for i, obj in enumerate(objectives):
            word_count = len(obj.split())
            if word_count < 15:
                issues.append(f"Objective {i+1} too short ({word_count} words) - PhD objectives need 20-35 words")
            elif word_count > 40:
                issues.append(f"Objective {i+1} too long ({word_count} words) - PhD objectives should be 20-35 words")
        
        return issues
    
    def _extract_objectives(self, response: str) -> List[str]:
        """Extract objectives from response for analysis."""
        try:
            clean_response = response.replace("```json", "").replace("```", "").strip()
            parsed = json.loads(clean_response)
            if isinstance(parsed, list):
                return [obj.split(":", 1)[1].strip() if ":" in obj else obj for obj in parsed]
        except:
            pass
        return []
