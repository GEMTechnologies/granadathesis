...
class VotingConfig:
    ...
    k: int = 2  # Votes ahead threshold (reduced from 3 for speed)
    max_samples: int = 8  # Maximum samples before giving up (reduced from 20 for speed)
    ...
    progress_callback: Optional[Callable] = None  # Callback for voting progress
    ...
    async def vote_until_consensus(
        ...
        while sample_count < self.config.max_samples:
            ...
            if self.config.progress_callback:
                import asyncio
                asyncio.create_task(self.config.progress_callback({
                    "sample": sample_count + 1,
                    "total_samples": self.config.max_samples,
                    "status": "generating",
                    "message": f"Generating sample {sample_count + 1}/{self.config.max_samples}..."
                }))
            ...
            try:
                ...
                # Emit real-time voting progress
                if self.config.progress_callback:
                    top_candidates = vote_counts.most_common(3)
                    # Get objectives preview for each candidate
                    candidate_previews = []
                    for cand_key, votes in top_candidates:
                        # Try to find parsed objectives for this candidate
                        cand_parsed = next(
                            (v["parsed"] for v in all_votes if not v["flagged"] and v["vote_for"] == cand_key),
                            None
                        )
                        if cand_parsed and isinstance(cand_parsed, list):
                            previews = [obj.split(":", 1)[1].strip()[:60] + "..." if ":" in obj else obj[:60] + "..." for obj in cand_parsed[:2]]
                        else:
                            previews = [str(cand_key)[:60] + "..."]
                        candidate_previews.append({"objectives": previews, "votes": votes})
                    
                    # Call progress callback (async)
                    import asyncio
                    asyncio.create_task(self.config.progress_callback({
                        "sample": sample_count,
                        "total_samples": self.config.max_samples,
                        "vote_distribution": {str(k)[:50]: v for k, v in dict(vote_counts).items()},
                        "top_candidates": candidate_previews,
                        "leading_votes": vote_counts.most_common(1)[0][1] if vote_counts else 0,
                        "needs_more": max(0, self.config.k - (vote_counts.most_common(1)[0][1] if vote_counts else 0))
                    }))
        ...
        # Max samples reached without consensus - ALWAYS return best candidate
        print(f"\n   ⚠️  MAX SAMPLES REACHED ({self.config.max_samples})")
        if vote_counts:
            # Return best candidate (even without full consensus)
            winner_key = vote_counts.most_common(1)[0][0]
            winner_votes = vote_counts[winner_key]
            winner_parsed = next(
                v["parsed"] for v in all_votes 
            ...
            if self.config.progress_callback:
                ...
                asyncio.create_task(self.config.progress_callback({
                    ...
                    "message": f"Voting complete: Best candidate selected with {winner_votes} votes"
                }))
